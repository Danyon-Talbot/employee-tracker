        ...
        connection.query('SELECT id, CONCAT(first_name, " ", last_name) AS name FROM employees', (err, employees) => {
            ...
            inquirer
                ...
                .then((answers) => {
                    ...
                    // Initialize manager_id as null by default
                    let manager_id = null;
...
function updateManager() {
    ...
    connection.query('SELECT id, CONCAT(first_name, " ", last_name) AS name FROM employees', (err, employees) => {
        ...
        // Add an option for not selecting a new manager
        employeeNames.push('None'); // You can customize the label as needed
        ...
        inquirer
            .prompt([
                ...
                },
                {
                    type: 'list', // Use a list type for manager selection
                    name: 'newManagerName',
                    message: 'Select the employee\'s new manager (or choose "None" for no manager):',
                    choices: employeeNames, // Display employee names as choices
                },
            ])
            .then((answers) => {
                ...
                // Determine if a new manager should be set
                let selectedNewManagerId = null;
                if (answers.newManagerName !== 'None') {
                    const selectedNewManager = employees.find((employee) => employee.name === answers.newManagerName);
                    selectedNewManagerId = selectedNewManager.id;
                }

                connection.query(
                    'UPDATE employees SET manager_id = ? WHERE id = ?',
                    [selectedNewManagerId, selectedEmployee.id],
                    (err) => {
                        if (err) throw err;
